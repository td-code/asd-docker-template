...
class ImageHandler:
    ...
    def __init__(self):
        ...
        # publishers
        self.pubDbgImage = rospy.Publisher(
...
class TrafficLightHandler:
    ...
    def __init__(self):
        ...
        self.trafficLightSub = rospy.Subscriber(
            "traffic_light_status", TrafficLightStatus, self.callback, queue_size=1
        )
    ...
    def callback(self, message):
        ...
        rospy.loginfo(
            "traffic light: state=%s, dist=%.1fm, dphi=%.1fdeg"
            % (
                state_dict[message.state],
                message.dist_m,
                message.dphi_rad * 180.0 / np.pi,
            )
        )
...
if __name__ == "__main__":
    ...
    joint_state_cache = message_filters.Cache(joint_state_sub, 100)
    ...
    tl_handler = TrafficLightHandler()
    ...
    while not rospy.is_shutdown():
        ...
        joint_state = joint_state_cache.getElemBeforeTime(time_now)
